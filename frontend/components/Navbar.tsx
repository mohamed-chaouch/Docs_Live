  ...
  return (
    <>
      <div className="flex items-center justify-between px-2 sm:px-12 my-2">
        ...
        {isDoc && setTitle && currentUserType === "editor" && (
          <div className="hidden md:flex items-center justify-center space-x-2">
            <input
              ...
              type="text"
              value={title}
              className="text-xl font-bold text-black bg-transparent outline-none w-fit text-right"
              style={{ width: `${Math.min(Math.max(title!.length * 12, 120), 250)}px` }}
              onChange={(e) => {
        ...
        {isDoc && setTitle && currentUserType !== "editor" && (
          <div className="hidden md:flex items-center justify-center space-x-2">
            <p className="text-xl font-bold text-black bg-transparent outline-none w-fit text-right truncate" style={{ width: `${Math.min(Math.max(title!.length * 12, 120), 250)}px` }}>
              {title}
            </p>
      ...
      {isDoc && setTitle && currentUserType === "editor" && (
        <div className="flex md:hidden items-center justify-center space-x-2 my-4">
          <input
            ...
            style={{ width: `${Math.min(Math.max(title!.length * 14, 130), 250)}px` }}
            onChange={(e) => {
              setTitle(e.target.value);
            }}
      ...
      {isDoc && setTitle && currentUserType !== "editor" && (
        <div className="flex md:hidden items-center justify-center my-4">
          <p className="text-2xl font-bold text-black bg-transparent outline-none w-fit text-center truncate"  style={{ width: `${Math.min(Math.max(title!.length * 14, 130), 250)}px` }}>
            {title}
          </p>
